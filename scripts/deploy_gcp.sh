...
COMMON_ENV="APP_ENV=production,STATE_BACKEND=firestore,GOOGLE_CLOUD_PROJECT=${GOOGLE_CLOUD_PROJECT},GOOGLE_CLOUD_LOCATION=${GOOGLE_CLOUD_LOCATION},AGENT_REGISTRY_LOCATION=${AGENT_REGISTRY_LOCATION},GOOGLE_GENAI_USE_VERTEXAI=true,FIRESTORE_DATABASE=(default),GEMINI_ROUTINE_MODEL=gemini-3.5-flash-lite,GEMINI_COMPLEX_MODEL=gemini-3.5-flash,MAX_MODEL_CALLS_PER_WORKFLOW=4,MAX_DAILY_WORKFLOWS=100,ENABLE_CLOUD_TRACE=true,PUBLIC_DASHBOARD=false,ENABLE_PUBLIC_DEMO=false,PUBLIC_DEMO_DAILY_LIMIT=10,PUBLIC_DEMO_TTL_MINUTES=20"
